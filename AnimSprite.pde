...
//~~~~~~~~~~~~~~~~~~~~~~
  ...
  void draw() {
    ...
    if(!isTexture){
      ...
      canvas.texture(frames[frameIndex]);
      canvas.vertex(vertices[0].x,vertices[0].y,uvs[0].x,uvs[0].y);
      canvas.vertex(vertices[1].x,vertices[1].y,uvs[1].x,uvs[1].y);
      canvas.vertex(vertices[2].x,vertices[2].y,uvs[2].x,uvs[2].y);
      canvas.vertex(vertices[3].x,vertices[3].y,uvs[3].x,uvs[3].y);
    ...
    if(debug){
        ...
        canvas.fill(255,0,0);
        canvas.ellipse(vertices[0].x,vertices[0].y,5,5);
        canvas.fill(0,255,0);
        canvas.ellipse(vertices[1].x,vertices[1].y,5,5);
        canvas.fill(0,0,255);
        canvas.ellipse(vertices[2].x,vertices[2].y,5,5);
        canvas.fill(255,0,255);
        canvas.ellipse(vertices[3].x,vertices[3].y,5,5);
      }
    }
    canvas.popMatrix();
  }
  ...
  void rotateXYZ(float _x, float _y, float _z) {
    canvas.rotateX(radians(_x));
    canvas.rotateY(radians(_y));
    canvas.rotateZ(radians(_z));
  }
  ...
  void blendImage(PImage bI, int pX, int pY, String b, boolean center) {
    ...
    for (int i=0;i<blendModes.length;i++) {
      if (b==blendModes[i]) {
        ...
        canvas.blend(bI, 0, 0, bI.width, bI.height, qX, qY, bI.width, bI.height, blendModeCodes[i]);
      }
    }
  }
